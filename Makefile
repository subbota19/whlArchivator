...
zip:
	@echo "Zipping $(PACKAGE_OUTPUT) directory to $(ZIP_PATH) with execution rights preserved..."
	@chmod -R 755 $(PACKAGE_OUTPUT)
	@cd $(PACKAGE_OUTPUT) && zip -j "$(ZIP_PATH)" *
	@if [ $$? -ne 0 ]; then echo "Error: Zipping $(PACKAGE_OUTPUT) failed."; exit 1; fi
	@echo "$(PACKAGE_OUTPUT) successfully zipped to $(ZIP_PATH)."
